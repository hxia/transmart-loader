...
class PostgresqlBioMarker extends BioMarker {
     ...
     * @param gene
     */
    void insertGenes(StringBuffer gene) {

        gene.toString().eachLine {
            String[] str = it.split(/\t/)
            // geneSymbol, description, geneId
            if(str.size() == 3)
                    insertBioMarker(str[1], str[2], str[0])
            else if(str.size() == 2)
                insertBioMarker(str[1], "", str[0])
            else
    ...
    /**
     * check if not exist, load it, otherwise do nothing
     *
     * @param geneSymbol
     * @param description
     * @param geneId
     */
    void insertBioMarker(String geneSymbol, String description, String geneId) {
        ...
        if (isBioMarkerExist(geneId, geneType)) {
            log.info "$organism:$geneSymbol:$geneId:$geneType already exists in BIO_MARKER ..."
        } else {
            biomart.execute(qry, [
                    geneSymbol,
                    description,
