...
class OraclePatientDimension extends PatientDimension {

    private static final Logger log = Logger.getLogger(OraclePatientDimension)

    void insertPatientDimension(String subjectId, int age, String gender, String vitalStatus) {

        String qry = """insert into patient_dimension(PATIENT_NUM, VITAL_STATUS_CD, SEX_CD, AGE_IN_YEARS_NUM, SOURCESYSTEM_CD, IMPORT_DATE)
                        values (SEQ_PATIENT_NUM.nextval, ?, ?, ?, ?, sysdate)"""

        if (isPatientDimensionExist(subjectId)) {
            log.info "Patient \"$subjectId\" already exists in PATIENT_DIMENSION ..."
        } else {
            log.info "Loading patient \"$subjectId\" into PATIENT_DIMENSION ..."
            i2b2demodata.execute(qry, [vitalStatus, gender, age, studyName + ":" + subjectId])
        }
    }
    ...
    Map getSubjectPatientMap() {
        ...
        i2b2demodata.eachRow(qry) {
            subjectPatientMap[it.sourcesystem_cd.replace(studyName + ":", "")] = it.patient_num
        }
    ...
    boolean isPatientDimensionExist(String subjectId) {
        String qry = "select count(*) from patient_dimension where sourcesystem_cd=?"
        def res = i2b2demodata.firstRow(qry, [studyName + ":" + subjectId])
